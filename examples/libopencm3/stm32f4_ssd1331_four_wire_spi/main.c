...
int main(void) {
	...
	for(uint16_t y=0;y<96;y++) {
		...
		eglib_DrawLine(&eglib,0, y, 96-1, y);
	}

	eglib_SetIndexColor(&eglib, 0, 255, 255, 255);
	eglib_DrawVLine(&eglib, 96/2, 0, 64);

	eglib_SetIndexColor(&eglib, 0, 255, 0, 0);
	eglib_DrawLine(&eglib, 0, 0, 96-1, 64/2);

	eglib_SetIndexColor(&eglib, 0, 0, 255, 0);
	eglib_DrawLine(&eglib,0, 0, 96-1, 64-1);

	eglib_SetIndexColor(&eglib, 0, 0, 0, 255);
	eglib_DrawLine(&eglib,0, 0, 96/2, 64-1);
}