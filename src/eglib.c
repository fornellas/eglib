...
void eglib_Init(
	...
	if(NULL == eglib->comm.power_up)
		eglib->comm.power_up = default_hal_power_up;
	if(NULL == eglib->comm.power_down)
		eglib->comm.power_down = default_hal_power_down;
	if(NULL == eglib->comm.delay)
		eglib->comm.delay = default_hal_delay;
	if(NULL == eglib->comm.set_reset)
		eglib->comm.set_reset = default_hal_set_reset;
	if(NULL == eglib->comm.set_cd)
		eglib->comm.set_cd = default_hal_set_cd;
	if(NULL == eglib->comm.set_cs)
		eglib->comm.set_cs = default_hal_set_cs;
	if(NULL == eglib->comm.send_byte)
		eglib->comm.send_byte = default_hal_send_byte;
