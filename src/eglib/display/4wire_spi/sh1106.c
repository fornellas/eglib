...
static inline void set_column_address(
	...
	eglib_hal_4wire_spi_send_byte(eglib, SH1106_SET_HIGHER_COLUMN_ADDRESS(column + display_config->column_offset));
	eglib_hal_4wire_spi_send_byte(eglib, SH1106_SET_LOWER_COLUMN_ADDRESS(column + display_config->column_offset));
}

static inline void display_on(eglib_t *eglib) {
	eglib_hal_4wire_spi_set_dc(eglib, 0);
	eglib_hal_4wire_spi_send_byte(eglib, SH1106_DISPLAY_ON);
	eglib_hal_4wire_spi_delay_ns(eglib, SH1106_DISPLAY_ON_MS * 1000 * 1000);
}
...
static void init(eglib_t *eglib) {
	...
	eglib_hal_4wire_spi_set_dc(eglib, 1);
	eglib_hal_4wire_spi_set_reset(eglib, 0);
	eglib_hal_4wire_spi_delay_ns(eglib, SH1106_RESET_LOW_MS * 1000 * 1000);
	eglib_hal_4wire_spi_set_reset(eglib, 1);
	eglib_hal_4wire_spi_delay_ns(eglib, SH1106_RESET_HIGH_MS * 1000 * 1000);
	...
	eglib_hal_4wire_spi_send_byte(eglib, SH1106_COMMON_PADS_HARDWARE_CONFIGURATION_MODE_SET);
	eglib_hal_4wire_spi_send_byte(eglib, SH1106_COMMON_PADS_HARDWARE_CONFIGURATION_MODE_SET_ARG(display_config->common_pads_hardware_configuration_mode));

	eglib_hal_4wire_spi_send_byte(eglib, SH1106_SET_COMMON_OUTPUT_SCAN_DIRECTION(display_config->common_output_scan_direction));	

	eglib_hal_4wire_spi_send_byte(eglib, SH1106_SET_DISPLAY_OFFSET(display_config->display_offset));

	// Change period

	eglib_hal_4wire_spi_send_byte(eglib, SH1106_DISCHARGE_PRECHARGE_PERIOD_MODE_SET);
	eglib_hal_4wire_spi_send_byte(eglib, SH1106_DISCHARGE_PRECHARGE_PERIOD_MODE_SET_ARG(display_config->dis_charge_period, display_config->pre_charge_period));	

	// VCOM deselect

	eglib_hal_4wire_spi_send_byte(eglib, SH1106_VCOM_DESELECT_LEVEL_MODE_SET);
	eglib_hal_4wire_spi_send_byte(eglib, display_config->vcom_deselect_level);


	// Internal display clocks

	eglib_hal_4wire_spi_send_byte(eglib, SH1106_SET_DISPLAY_CLOCK_DIVIDE_RATIO_OSCILLATOR_FREQUENCY);
	eglib_hal_4wire_spi_send_byte(eglib, SH1106_SET_DISPLAY_CLOCK_DIVIDE_RATIO_OSCILLATOR_FREQUENCY_ARG(display_config->oscillator_frequency, display_config->clock_divide));	

	// Charge Pump Regulator
	if(display_config->dc_dc_enable) {
		eglib_hal_4wire_spi_send_byte(eglib, SH1106_DC_DC_CONTROL_MODE_SET);
		eglib_hal_4wire_spi_send_byte(eglib, SH1106_DC_DC_ON);
		eglib_hal_4wire_spi_send_byte(eglib, SHH1106_SET_PUMP_VOLTAGE(display_config->dc_dc_voltage));
	} else {
		eglib_hal_4wire_spi_send_byte(eglib, SH1106_DC_DC_CONTROL_MODE_SET);
		eglib_hal_4wire_spi_send_byte(eglib, SH1106_DC_DC_OFF);
	}

	// Clear RAM
	for(uint8_t page=0 ; page < (display_config->height / 8) ; page++) {
		eglib_hal_4wire_spi_set_dc(eglib, 0);
		eglib_hal_4wire_spi_send_byte(eglib, SH1106_SET_PAGE_ADDRESS(page));
		set_column_address(eglib, 0);
		eglib_hal_4wire_spi_set_dc(eglib, 1);
		for(eglib_coordinate_t column=0 ; column < display_config->width ; column ++)
			eglib_hal_4wire_spi_send_byte(eglib, 0x00);
	}
